...
def partial_coded_logistic_regression(n_procs, n_samples, n_features, input_dir, n_stragglers, n_partitions, is_real_data, params):
    ...
    if (rank):
        ...
        if not is_real_data:
            ...
            for i in range(n_separate,n_partitions):
                ...
                idx = n_separate*n_workers + (rank-1+i-n_separate)%n_workers
    ...
    if rank:
        ...
        msgBuffers_secondparts = np.array([np.zeros(n_features) for i in range(n_procs-1)])
        ...
        completed_stragglers=np.ndarray(n_workers,dtype=bool)
    ...
    if (rank):
        y_current_mod=np.zeros(n_partitions*rows_per_worker)
        y_current_mod[:] = y_current

        for i in range(n_separate,n_partitions):        
                y_current_mod[i*rows_per_worker:(i+1)*rows_per_worker] = B[rank-1, (rank-1+i-n_separate)%n_workers]*y_current[i*rows_per_worker:(i+1)*rows_per_worker]
    ...
    comm.Barrier()
    if rank==0:
        print("---- Starting Partial Coded Iterations for " +str(n_stragglers) + " stragglers ----")
    ...
    for i in range(rounds):
        if rank==0:
            ...
            g += np.squeeze(np.dot(A_row, msgBuffers_secondparts))
            ...
            g_secondpart = X_current[sep_lim:,:].T.dot(np.divide(y_current_mod[sep_lim:],np.exp(np.multiply(predy,y_current[sep_lim:]))+1))
            g_secondpart *= -1
    ...
    if rank==0:
        ...
        save_vector(testing_loss, output_dir+"partialcoded_%d_%d_testing_loss.dat"%(n_stragglers,n_partitions))
        save_vector(auc_loss, output_dir+"partialcoded_%d_%d_auc.dat"%(n_stragglers,n_partitions))
        save_vector(timeset, output_dir+"partialcoded_%d_%d_timeset.dat"%(n_stragglers,n_partitions))
        save_matrix(worker_timeset, output_dir+"partialcoded_%d_%d_worker_timeset.dat"%(n_stragglers,n_partitions))
        print(">>> Done")
